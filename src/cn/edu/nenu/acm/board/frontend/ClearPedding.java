 ...
 */
@WebServlet(urlPatterns = { "/ClearPedding" }, initParams = { @WebInitParam(name = "tid", value = "", description = "Team ID In Database") })
		...
		synchronized (Board.dbMutex) {
			...
			if (login == null) {
				...
				try {
					...
					pstm = conn
							.prepareStatement("UPDATE Runs SET rDescription='',rLastUpdateTime=? WHERE rDescription='F'");
					pstm.setLong(1, DBStatusFetcher.getLastUpdateTime()
							+ Board.statusCacheInterval * 10);
					pstm.execute();
					pstm.close();
					conn.close();
					result = "{\"code\":0}";
				...
				try {
					...
					long updateTime = DBStatusFetcher.getLastUpdateTime()
							+ Board.statusCacheInterval * 10;
					while (rs.next()) {
						int status = rs.getInt("rStatus");
						rs.updateString("rDescription", "F");
						rs.updateLong("rLastUpdateTime", updateTime++);// 考虑这里更新很慢的因素，所以为保证更新过程中不会让一些东西无法被长连接忽略掉，每次都加一
						rs.updateRow();
						if (status == Board.RUNSTATUS_YES) {
							int pid = rs.getInt("rPId");
							while (rs.next()) {
								if (pid != rs.getInt("rPId"))
									break;
								rs.updateString("rDescription", "F");
				...
				try {
					...
					Connection conn = Board.getDataBaseConnection();
					PreparedStatement pstm = conn
							.prepareStatement("UPDATE Runs SET rDescription=?,rLastUpdateTime=? WHERE rTId=? AND rPId=? AND rTime>=?");
					pstm.setString(1, "F");// 标记为Final，跳过Pedding缓存处理
					pstm.setLong(2, DBStatusFetcher.getLastUpdateTime()
							+ Board.statusCacheInterval * 10);// 标记更新时间戳，随便制定一个10倍之后的。实际上，现在系统已经和时钟没关系了，所以随便就可以。
					pstm.setInt(3, tid);
					pstm.setInt(4, pid);
